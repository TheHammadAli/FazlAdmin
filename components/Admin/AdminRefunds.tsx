...
function AdminRefunds() {
    ...
    useEffect(() => {
        setPage(1);
    }, [refundStatus, dateFilter, customStartDate, customEndDate]);
    ...
    const response = data as PaginatedResponse<RefundRow> | undefined;
    ...
    return (
        <section>
            <CreateRefundModal open={createOpen} onClose={() => setCreateOpen(false)} />
            <RefundDetailModal refundId={viewingId} onClose={() => setViewingId(null)} />
            ...
            <div className="bg-[#F6F8FA] pt-6 pb-5">
                <div className="container mx-auto flex flex-wrap items-start justify-between gap-3 px-5 lg:px-10">
                    <p className="text-[13px] font-normal text-gray-11">
                        View and manage refunds — completing one automatically credits the wallet ledger
                    </p>
                    <DoodleButton
                        type="button"
                        ...
                        <table className="w-full min-w-[860px]">
                            <thead>
                                <tr className="text-left">
                                    <th className="py-3 pr-4 text-[14px] font-medium text-[#001907]">Refund ID</th>
                                    <th className="py-3 pr-4 text-[14px] font-medium text-[#001907]">Customer</th>
                            ...
                            <tbody>
                                ...
                                {!loading &&
                                    rows.map((r) => (
                                        <tr key={r._id} className="border-t border-gray-10">
                                            ...
                                            <td className="whitespace-nowrap py-3.5 pr-4 text-[13px] text-[#001907]">
                                                {refName(r.customerId)}
                                            </td>
                                            <td className="whitespace-nowrap py-3.5 pr-4 text-[13px] text-[#001907]">
                                                {refName(r.merchantId)}
                                            </td>
                                            <td className="whitespace-nowrap py-3.5 pr-4 text-[13px] text-[#001907]">
                                                {formatMoneyMinor(r.refundAmountMinor)}
                                            </td>
                                            <td className="whitespace-nowrap py-3.5 pr-4">
                                                <span
                                                    className={`rounded-[4px] px-2 py-0.5 text-[12px] font-medium ${STATUS_COLORS[r.refundStatus] ?? "bg-gray-10 text-gray-8"
                                                        }`}
                                                >
                                                    {r.refundStatus}
                                                </span>
                                            ...
                                            <td className="whitespace-nowrap py-3.5 pr-4 text-[13px] text-gray-11">
                                                {fmtDate((r as unknown as { createdAt?: string }).createdAt)}
                                            </td>
