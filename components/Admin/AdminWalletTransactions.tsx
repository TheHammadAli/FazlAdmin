...
import { useGetWalletTransactionsQuery } from "@/store/services/adminService";
import Pagination from "@/components/Ui/Pagination";
import DateRangeFilter, { type DateFilterValue } from "@/components/Ui/DateRangeFilter";
import WalletTransactionDetailModal from "@/components/Admin/WalletTransactionDetailModal";
import { getDateRangeForFilter } from "@/utils/getDateRangeForFilter";
import { formatMoneyMinor } from "@/utils/formatMoney";
import searchIcon from "@/assets/icons/searchIcon.svg";
import type { PaginatedResponse, WalletTransactionRow } from "@/store/services/walletTypes";

const PAGE_LIMIT = 20;
const SEARCH_DEBOUNCE_MS = 400;

const STATUS_OPTIONS = ["", "pending", "completed", "failed", "reversed"];
const PAYMENT_METHOD_OPTIONS = ["", "fazl_wallet", "cash"];
...
const STATUS_COLORS: Record<string, string> = {
    ...
    failed: "bg-[#FDD5D5] text-[#E92440]",
    reversed: "bg-gray-10 text-gray-8",
};
...
function AdminWalletTransactions() {
    ...
    useEffect(() => {
        setPage(1);
    }, [status, paymentMethod, dateFilter, customStartDate, customEndDate]);
    ...
    const response = data as PaginatedResponse<WalletTransactionRow> | undefined;
    ...
    return (
        <section>
            <WalletTransactionDetailModal transactionId={viewingId} onClose={() => setViewingId(null)} />

            <div className="bg-[#F6F8FA] pt-6 pb-5">
                <div className="container mx-auto px-5 lg:px-10">
                    <p className="text-[13px] font-normal text-gray-11">
                        Full searchable, filterable list of every wallet transaction
                    </p>
                </div>
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        <div className="relative">
                            <select
                                ...
                                className="h-10 min-w-[140px] appearance-none rounded-[8px] border border-gray-9 bg-white pl-3 pr-8 text-[14px] text-[#001907] outline-none focus:border-green-1"
                            >
                        ...
                        <table className="w-full min-w-[980px]">
                            <thead>
                                <tr className="text-left">
                                    <th className="py-3 pr-4 text-[14px] font-medium text-[#001907]">Transaction ID</th>
                                    ...
                                    <th className="py-3 pr-4 text-[14px] font-medium text-[#001907]">Refund</th>
                                    <th className="py-3 pr-4 text-[14px] font-medium text-[#001907]">Date</th>
                            ...
                            <tbody>
                                ...
                                {!loading && rows.length === 0 && (
                                    <tr>
                                        <td colSpan={9} className="py-8 text-center text-[14px] text-gray-11">
                                ...
                                {!loading &&
                                    rows.map((txn) => (
                                        <tr key={txn._id} className="border-t border-gray-10">
                                            <td className="whitespace-nowrap py-3.5 pr-4 text-[13px] text-gray-11">
                                                {txn.transactionCode ?? txn._id}
                                            </td>
                                            <td className="whitespace-nowrap py-3.5 pr-4 text-[13px] text-[#001907]">
                                                {refName(txn.userId)}
                                            </td>
                                            <td className="whitespace-nowrap py-3.5 pr-4 text-[13px] text-[#001907]">
                                                {refName(txn.merchantId)}
                                            </td>
                                            <td className="whitespace-nowrap py-3.5 pr-4 text-[13px] text-[#001907]">
                                                {formatMoneyMinor(txn.finalCustomerPaymentMinor || txn.originalAmountMinor)}
                                            </td>
                                            <td className="whitespace-nowrap py-3.5 pr-4 text-[13px] text-gray-11">
                                                {txn.paymentMethod}
                                            </td>
                                            <td className="whitespace-nowrap py-3.5 pr-4">
                                                <span
                                                    className={`rounded-[4px] px-2 py-0.5 text-[12px] font-medium ${STATUS_COLORS[txn.status] ?? "bg-gray-10 text-gray-8"
                                                        }`}
                                                >
                                                    {txn.status}
                                                </span>
