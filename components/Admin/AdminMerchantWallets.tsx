...
function AdminMerchantWallets() {
    ...
    return (
        <section>
            <WalletDetailModal
                ownerId={viewingMerchantId}
                walletType="merchant"
                onClose={() => setViewingMerchantId(null)}
            />

            <div className="bg-[#F6F8FA] pt-6 pb-5">
                <div className="container mx-auto px-5 lg:px-10">
                    <p className="text-[13px] font-normal text-gray-11">
                        Search merchants and service providers — same wallet system for both
                    </p>
                </div>
                        ...
                        <table className="w-full min-w-[900px]">
                            <thead>
                                <tr className="text-left">
                                    ...
                                    <th className="py-3 pr-4 text-[14px] font-medium text-[#001907]">Total Received</th>
                                    <th className="py-3 pr-4 text-[14px] font-medium text-[#001907]">Total Withdrawn</th>
                                            ...
                                            <tr key={id} className="border-t border-gray-10">
                                                <td className="py-3.5 pr-4">
                                                    <div className="flex items-center gap-3">
                                                        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#F1E9FE] text-[11px] font-medium text-[#030303]">
                                                ...
                                                <td className="whitespace-nowrap py-3.5 pr-4 text-[14px] text-gray-11">
                                                    {row.wallet ? formatMoneyMinor(row.wallet.totalReceivedMinor) : "-"}
                                                </td>
                                                <td className="whitespace-nowrap py-3.5 pr-4 text-[14px] text-gray-11">
                                                    {row.wallet ? formatMoneyMinor(row.wallet.totalWithdrawnMinor) : "-"}
                                                </td>
                                                ...
                                                <td className="py-3.5 text-center">
                                                    <button
                                                        type="button"
                                                        onClick={() => setViewingMerchantId(id)}
...
export default AdminMerchantWallets;
