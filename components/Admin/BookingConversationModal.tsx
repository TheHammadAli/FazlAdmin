...
import { useGetBookingConversationQuery } from "@/store/services/adminService";
...
type ApiConversationData = {
    conversation?: { _id?: string; status?: string } | null;
    messages?: ApiMessage[];
    meta?: { total?: number; page?: number; limit?: number; totalPages?: number };
};

type ApiConversationResponse = { data?: ApiConversationData };
...
type BookingConversationModalProps = {
    open: boolean;
    customerId?: string;
    providerId?: string;
    customerName?: string;
...
function BookingConversationModal({
    open,
    customerId,
    providerId,
    customerName,
    ...
    const { data, isLoading, isFetching } = useGetBookingConversationQuery(
        { customerId: customerId ?? "", providerId: providerId ?? "", page, limit: MESSAGES_PER_PAGE },
        { skip: !open || !customerId || !providerId },
    );
    const loading = isLoading || isFetching;

    const response = (data as ApiConversationResponse | undefined)?.data;
            ...
            <div className="hide-scrollbar flex max-h-[85vh] w-[92vw] max-w-[520px] flex-col rounded-[12px] bg-white shadow-xl">
                <div className="shrink-0 border-b border-gray-9 px-6 pt-6 pb-4">
                    ...
                    <p className="mt-1 text-[12px] text-gray-11">
                        {customerName ?? "Customer"} &amp; {providerName ?? "Provider"}
                    </p>
                </div>
                ...
                <div className="hide-scrollbar flex-1 overflow-y-auto px-6 py-5">
                    {loading ? (
                        ...
                        <p className="py-8 text-center text-[13px] text-gray-11">
                            No conversation yet between the customer and provider.
                        </p>
                    ) : (
                        <div className="space-y-3">
                            {messages.map((message) => {
                                const isFromCustomer = message.sender === customerId;
                                return (
                                    <div
                                        key={message._id}
                                    ...
                                    >
                                        <div
                                            className={`max-w-[75%] rounded-[10px] px-3 py-2 ${isFromCustomer ? "bg-gray-10 text-[#001907]" : "bg-green-4 text-[#001907]"
                                                }`}
                                        >
                                            <p className="text-[11px] font-medium text-gray-11">
                                                {isFromCustomer ? customerName ?? "Customer" : providerName ?? "Provider"}
                                            </p>
                                            {message.text && (
                ...
                {!loading && conversation && messages.length > 0 && totalPages > 1 && (
...
export default BookingConversationModal;
